...
class Neo4jStore(GraphStore):
    def __init__(self, uri: str, user: str, password: str, database: str = "neo4j"):
        ...
        self._write_tx = None   # 当前写事务（HybridStore 两阶段提交时使用）
        self.init_schema()

    def close(self):
        self.driver.close()

    def _run_read(self, cypher: str, **params):
        """只读查询（自动提交）。"""
        with self.driver.session(database=self.database) as s:
            return list(s.run(cypher, **params))

    def _run_write(self, cypher: str, **params):
        """写操作：如果在事务中则复用事务，否则自动提交。"""
        if self._write_tx is not None:
            # 在 HybridStore 管理的写事务中
            return list(self._write_tx.run(cypher, **params))
        else:
            with self.driver.session(database=self.database) as s:
    ...
    def init_schema(self) -> None:
        self._run_write("CREATE CONSTRAINT entity_id IF NOT EXISTS "
                        "FOR (e:Entity) REQUIRE e.id IS UNIQUE")
        self._run_write("CREATE INDEX entity_name IF NOT EXISTS FOR (e:Entity) ON (e.name)")
        self._run_write("CREATE INDEX entity_type IF NOT EXISTS FOR (e:Entity) ON (e.type)")

    # ---------- 事务控制（供 HybridStore 两阶段提交） ----------
    def begin_write_transaction(self):
        """开启一个写事务。"""
        self._session = self.driver.session(database=self.database)
        self._write_tx = self._session.begin_transaction()

    def commit_write_transaction(self):
        """提交写事务。"""
        if self._write_tx is not None:
            self._write_tx.commit()
            self._write_tx = None
        if hasattr(self, "_session") and self._session is not None:
            self._session.close()
            self._session = None

    def rollback_write_transaction(self):
        """回滚写事务。"""
        if self._write_tx is not None:
            self._write_tx.rollback()
            self._write_tx = None
        if hasattr(self, "_session") and self._session is not None:
            self._session.close()
            self._session = None
    ...
    def delete_entity(self, eid: str) -> bool:
        rows = self._run_write(
            "MATCH (n:Entity {id:$id}) DETACH DELETE n RETURN count(*) AS c", id=eid)
        return rows[0]["c"] > 0
    ...
    def list_entities(self, type_, keyword, page, size) -> Tuple[int, List[Entity]]:
        ...
        if keyword:
            where.append(
                "(n.name CONTAINS $kw OR n.alias CONTAINS $kw OR n.description CONTAINS $kw)"
            )
            params["kw"] = keyword
        w = " AND ".join(where)
        total = self._run_read(
            f"MATCH (n:Entity) WHERE {w} RETURN count(n) AS c", **params)[0]["c"]
        rows = self._run_read(
    ...
    def next_id(self, type_: str) -> str:
        ...
        rows = self._run_read(
            "MATCH (n:Entity) WHERE n.id STARTS WITH $p RETURN n.id AS id "
    ...
    def list_relations(self, source_id, target_id, relation, page, size):
        ...
        total = self._run_read(f"{base} RETURN count(e) AS c", **params)[0]["c"]
        rows = self._run_read(
    ...
    def neighbors(self, eid, relation, limit):
        ...
        rows = self._run_read(cy, id=eid, rel=relation, lim=limit)
    ...
    def find_paths(self, source_id, target_id, max_depth, limit=5):
        ...
        for row in rows:
            ...
            rels = [Relation(source_id=e.start_node["id"],
                             source_name=e.start_node["name"],
                             relation=e["relation"],
    ...
    def clear(self) -> None:
        self._run_write("MATCH (n:Entity) DETACH DELETE n")

    def stats(self) -> dict:
        et = {r["t"]: r["c"] for r in
              self._run_read("MATCH (n:Entity) RETURN n.type AS t, count(*) AS c")}
        rt = {r["t"]: r["c"] for r in
              self._run_read("MATCH ()-[e:REL]->() RETURN e.relation AS t, count(*) AS c")}
