...
首次运行以及知识图谱数据更新后，先从仓库根目录同步完整图谱：

```bash
python member3/scripts/sync_graph_data.py
```

该脚本以 `data/entities` 和 `data/relations` 为唯一数据源，生成
GraphRAG 使用的 `member3/data/entities.json` 和
`member3/data/relations.json`，并检查重复 ID、重复关系和悬空端点。
